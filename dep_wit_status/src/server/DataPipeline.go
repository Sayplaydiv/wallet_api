...
func Insert() {
	//冲币状态入库
	dep_result := dbConnect.InitDB_dep()
	for i := 0; i < len(dep_result); i++ {
			...
			user_id := dbConnect.InitDB_audit("deposit",dep_address)
			//fmt.Println("dep_confirm_height:",dep_confirm_height)
			if user_id !=0 {
				fmt.Println(dep_tx_hash)
				//asset string,address string,user_id int,amount float64,tx_hash string,block_height int,tx_time time.Time,confirm int
				dbConnect.InitDB_insert(dep_asset,dep_address,user_id,dep_amount,dep_tx_hash,dep_block_height,dep_confirm,dep_confirm_height)
			}

	}

	//提币状态入库
	wit_result := dbConnect.InitDB_wit()
	for i :=0;i<len(wit_result) ; i++ {
		...
		user_id := dbConnect.InitDB_audit("withdraw",wit_id)
		//fmt.Println(user_id)
		if user_id == 1000000000000 {
			//asset string,address string,user_id int,amount float64,tx_hash string,block_height int,tx_time time.Time,confirm int
			dbConnect.InitDB_insert(wit_asset,wit_address,user_id,wit_amount,wit_tx_hash,0,2,0)
		}
	}
}
