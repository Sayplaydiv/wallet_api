...
func InitDB_dep()  (map[int]map[string]string){
	...
	//查询数据，取所有字段
	rows2, _ := DB.Query("select * from deposit_key where status=1");
	//返回所有列
...
func InitDB_wit()  (map[int]map[string]string){
	...
	//查询数据，取所有字段
	rows2, _ := DB.Query("select * from withdraw_key where process<4 and process>=0 and status<4");
	//返回所有列
	cols, _ := rows2.Columns();
	//这里表示一行所有列的值，用[]byte表示
	vals := make([][]byte, len(cols));
...
func InitDB_wallet_delete(method string,tx_hash string)(int){
	...
	case "select_depsoit":
		var status int
		err := DB.QueryRow("select status from deposit_key where tx_hash= ?", tx_hash).Scan(&status)
		if err != nil {
	...
	case "select_deposit_height":
		...
		err := DB.QueryRow("select confirm_height from deposit_key where tx_hash= ?", tx_hash).Scan(&confirm_height)
		if err != nil {
			//fmt.Println(err.Error())
		} else if err == sql.ErrNoRows {
			//fmt.Println("no rows in result set")
	...
	case "select_witdraw_status":
		var status int
		err := DB.QueryRow("select status from withdraw_key where txhash= ?", tx_hash).Scan(&status)
		if err != nil {
			//fmt.Println(err.Error())
		} else if err == sql.ErrNoRows {
			//fmt.Println("no rows in result set")
