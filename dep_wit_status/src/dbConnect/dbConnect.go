...
//api链接返回
//asset string,address string,user_id int,amount float64,tx_hash string,block_height int,confirm int
func InitDB_wallet(user_id int,confirm int)(map[int]map[string]string){
	var DB *sql.DB
	...
	if err := DB.Ping(); err != nil{
		...
		//查询数据，取所有字段
		rows2, _ := DB.Query("select * from depwit_status where confirm=? and user_id=?",confirm,user_id);
		//返回所有列
...
func InitDB_dep()  (map[int]map[string]string){
	...
	//查询数据，取所有字段
	rows2, _ := DB.Query("select * from deposit where status=1");
	//返回所有列
	cols, _ := rows2.Columns();
	//这里表示一行所有列的值，用[]byte表示
	vals := make([][]byte, len(cols));
...
func InitDB_wit()  (map[int]map[string]string){
	...
	//查询数据，取所有字段
	rows2, _ := DB.Query("select * from withdraw where process<4 and process>=0 and status<4");
	//返回所有列
	cols, _ := rows2.Columns();
	//这里表示一行所有列的值，用[]byte表示
	vals := make([][]byte, len(cols));
...
func InitDB_insert(asset string,address string,user_id int,amount float64,tx_hash string,block_height int,confirm int,confirm_height int)(){
	var DB *sql.DB
	//fmt.Println("sql confirm count:",confirm_height)
...
func InitDB_wallet_delete(method string,tx_hash string)(int){
	...
	case "select_depsoit":
		var status int
		err := DB.QueryRow("select status from deposit where tx_hash= ?", tx_hash).Scan(&status)
		if err != nil {
	...
	case "select_deposit_height":
		...
		err := DB.QueryRow("select confirm_height from deposit where tx_hash= ?", tx_hash).Scan(&confirm_height)
		if err != nil {
			//fmt.Println(err.Error())
		} else if err == sql.ErrNoRows {
			//fmt.Println("no rows in result set")
	...
	case "select_witdraw_status":
		var status int
		err := DB.QueryRow("select status from withdraw where txhash= ?", tx_hash).Scan(&status)
		if err != nil {
			//fmt.Println(err.Error())
		} else if err == sql.ErrNoRows {
			//fmt.Println("no rows in result set")
...
func InitDB_wallet_update(select_deposit_height int,check_tx_hash string){
	var DB *sql.DB
	fmt.Println("块高更新：",select_deposit_height)
	//导入配置文件
	configMap := conf.InitConfig("/opt/dep_wit_status/src/config/conf.ini")
	//获取配置value
	userName_audit:=configMap["userName_audit"]
	...
	fmt.Println("update数据：",select_deposit_height)
