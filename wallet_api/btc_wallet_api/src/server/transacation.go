...
func Transaction(from_address string,to_address string,Value float64,Privatekey_1 string,Privatekey_2 string){
	...
	*/
	...
	var body_utxo hexstring
	json.Unmarshal(body,&body_utxo)
	body_utxo_result := string(body_utxo.Result)



	if body_utxo_result =="" {
		params := []interface{}{minconf, maxconf, unspen_address, true}
		...
		if err := json.Unmarshal([]byte(body), &dat); err == nil {
			...
			for i,data :=range Result{
				...
				utxo_amount :=utxo_data["amount"].(float64)
				...
				if  data != nil && i==0 {
					...
					vout_value :=utxo_amount-Value-0.0002
					...
					hexstring_bool :=string(hexstring_0)

					if hexstring_bool == "" {
						fmt.Println("创建交易失败，请根据上面的输出排查原因")
